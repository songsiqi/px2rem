...
Px2rem.prototype.generateRem = function (cssText) {
  ...
  function processRules(rules, noDealPx) { // FIXME: keyframes do not support `force px` comment
    for (var i = 0; i < rules.length; i++) {
      ...
      var remHackMeida = self._getRemHackMeida(rule)
          ...
          if (nextDeclaration && nextDeclaration.type === 'comment') { // next next declaration is comment
            if (nextDeclaration.comment.trim() === config.forcePxComment) { // force px
              ...
              if (!noDealPx) {
                ...
                remHackMeida.addDeclaration(declaration)
      ...
      if (remHackMeida.mediaSubRule.declarations.length) {
        rules.splice(i + 1, 0, remHackMeida.mediaRule)
...
// get meida rule for hack rem
Px2rem.prototype._getRemHackMeida = function (rule) {
  var config = this.config
  var mediaRule = {}
  var mediaSubRule = {}
  var result = null

  mediaRule.type = 'media'
  mediaRule.media = config.remHackMeida.media
  mediaRule.rules = []
  mediaSubRule.type = rule.type
  mediaSubRule.selectors = rule.selectors
  mediaSubRule.declarations = []
  mediaRule.rules.push(mediaSubRule)

  result = {
    'mediaRule': mediaRule,
    'mediaSubRule': mediaSubRule,
  }

  var targetPropertys = config.remHackMeida.propertys
  result.addDeclaration = function (declaration) {
    if (!config.remHackMeida.enabled) return
    var matched = false
    for (var i = 0; i < targetPropertys.length; i++) {
      if (targetPropertys[i] === declaration.property) {
        matched = true
        break
      }
    }
    if (!matched) return
    var mediaDeclaration = {}
    extend(true, mediaDeclaration, declaration)
    mediaSubRule.declarations.push(mediaDeclaration)
  }
