...
var defaultConfig = {
  ...
  keepValue: '',          // no transform value (like: `1px`, use regExp or string)
  keepProperty: ''        // no transform property (like: `border`, use regExp or string)
};
  ...
  function processRules(rules, noDealPx) { // FIXME: keyframes do not support `force px` comment
    for (var i = 0; i < rules.length; i++) {
      ...
      for (var j = 0; j < declarations.length; j++) {
        var declaration = declarations[j];
        // need transform: declaration && has 'px'
        if (config.keepValue && new RegExp(config.keepValue).test(declaration.value)) {
          // value match user's options, keep value
          continue;
        }
        if (config.keepProperty && new RegExp(config.keepProperty).test(declaration.property)) {
          // property match user's options, keep value
          continue;
        }
        if (declaration.type === 'declaration' && pxRegExp.test(declaration.value)) {
